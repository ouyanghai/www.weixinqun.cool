...
class Controller extends CController
{
	...
	public function uploadFile($path='upload'){
		...
		$date = date("Y-m-d");
		/*
		$dirname = Yii::app()->basePath."/../httpdocs/images/{$path}";
		if(!is_dir($dirname))
			mkdir($dirname,0777,true);
		*/
		$dirname = Yii::app()->basePath."/../images/{$path}/{$date}";
		if(!is_dir($dirname))
			mkdir($dirname,0777,true);
		...
		foreach ($files as $key => $value) {
			if($value['size'] > $this->maxFileSize || !in_array($value['type'], $this->fileType)){
				return array();
			}
			$filename = date('Y-m-d',time())."_{$num}_".md5(uniqid());
			$ext = strrchr($value['name'],'.');
			$filename .= $ext;
			$num++;
			if(move_uploaded_file($value['tmp_name'],$dirname.'/'.$filename)){
