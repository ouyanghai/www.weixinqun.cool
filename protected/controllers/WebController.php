...
class WebController extends TopController{
	...
	public function actionIndex(){
		...
		$res1 = $command->setText("select id,qun_logo,qun_name,created from `qun_img` where status=1 and qun_cate<10 order by created desc limit 0,16")->queryAll();

		$res2 = $command->setText("select id,qun_logo,qun_name,created from `qun_img` where status=1 and qun_cate>9 and qun_cate<20  order by created desc limit 0,16")->queryAll();

		$res3 = $command->setText("select id,qun_logo,qun_name,created from `qun_img` where status=1 and qun_cate>19 and qun_cate<28  order by created desc limit 0,10")->queryAll();
	...
	public function actionGetMobileFileInfo(){
		...
		if(!isset($_GET['cate_name']) || empty($_GET['city_name']) || empty($_GET['qun_name']) || empty($_GET['qun_logo'])|| empty($_GET['qun_user_img'])|| empty($_GET['qun_img'])|| empty($_GET['user_wx'])|| empty($_GET['qun_info'])|| empty($_GET['qun_keyword'])){
			echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("参数错误").")" : json_encode("参数错误");exit;
		}
		$date = date("Y-m-d H:i:s");
		...
		$qun_cate = $this->getCateIdByName($cate_name);
		if(empty($qun_cate)){
			echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("参数行业名称错误").")" : json_encode("参数行业名称错误");exit;
		}
		$qun_city = $this->getCityIdByName($city_name);
		if(empty($qun_city)){
			echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("参数城市名称{$qun_city}错误").")" : json_encode("参数城市名称{$qun_city}错误");exit;
		}

		$user_wx  = trim($_GET['user_wx']);
		$qun_name = trim($_GET['qun_name']);
		$qun_info = trim($_GET['qun_info']);
		$keyword = str_replace(" ", ",", $_GET['qun_keyword']);

		$qun_logo = trim($_GET['qun_logo']);

		$qun_user_img = trim($_GET['qun_user_img']);

		$qun_img = trim($_GET['qun_img']);
		...
		$num = $command->setText($sql)->execute();
		echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("ok").")" : json_encode("ok");exit;
	}

	public function actionGetMobilePersonInfo(){
		
		if( empty($_GET['city_name']) || empty($_GET['name']) ||  empty($_GET['user_img'])|| empty($_GET['user_wx'])|| empty($_GET['info']) || empty($_GET['sex'])){
			echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("参数错误").")" : json_encode("参数错误");exit;
		}
		$date = date("Y-m-d H:i:s");
		...
		$user_city = $this->getCityIdByName($city_name);
		if(empty($user_city)){
			echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("参数城市名称{$user_city}错误").")" : json_encode("参数城市名称{$user_city}错误");exit;
		}
		...
		$user_img = trim($_GET['user_img']);

		$command = Yii::app()->db->createCommand();
		$sql = "insert into `user_img` (user_wx,user_city,city_name,name,info,user_img,status,created,updated,sex) values('{$user_wx}','{$user_city}','{$city_name}','{$name}','{$info}','{$user_img}',0,'{$date}','{$date}','{$sex}')";
		$num = $command->setText($sql)->execute();
		echo isset($_GET['callback']) ? $_GET['callback']."(".json_encode("ok").")" : json_encode("ok");exit;
	}
